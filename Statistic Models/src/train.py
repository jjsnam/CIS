...
def train_loop(train_real_dir, train_fake_dir, val_real_dir, val_fake_dir):
    init_logging()

    # Load or extract features
    X_real_train = load_features_from_folder(train_real_dir, "features/real_train.npy")
    X_fake_train = load_features_from_folder(train_fake_dir, "features/fake_train.npy")
    X_real_val = load_features_from_folder(val_real_dir, "features/real_val.npy")
    X_fake_val = load_features_from_folder(val_fake_dir, "features/fake_val.npy")
    ...
    for epoch in range(1, 11):
        logger.info(f"Epoch {epoch} - Training GMMs")
        ...
        plt.title(f"Epoch {epoch} - Validation Log-Likelihood Score Distribution")
        plt.xlabel("log(P_real) - log(P_fake)")
        plt.ylabel("Count")
        plt.legend()
        os.makedirs("plots", exist_ok=True)
        plt.savefig(f"plots/val_score_epoch{epoch}.png")
        plt.close()


        acc = accuracy_score(y_val, y_pred)
        logger.info(f"Epoch {epoch} - Val Accuracy: {acc:.4f}")
        ...
        if acc > best_acc:
            ...
            model.save("checkpoints/gmm_real.pkl", "checkpoints/gmm_fake.pkl")
            best_acc = acc